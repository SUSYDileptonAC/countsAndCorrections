...
if (__name__ == "__main__"):
	...
	hCanvas.Clear()
	legend.Clear()
	cutsLowMET = "weight*(chargeProduct < 0 && %s && met < 50 && nJets >=2  && abs(eta1) < 2.4 && abs(eta2) < 2.4 && deltaR > 0.3)"%ptCut
	cutsLowJets = "weight*(chargeProduct < 0 && %s && met > 50 && nJets >=2  && abs(eta1) < 2.4 && abs(eta2) < 2.4 && deltaR > 0.3)"%ptCut
	for name, tree in EEtrees.iteritems():
		if name == sampleName:
			print name
