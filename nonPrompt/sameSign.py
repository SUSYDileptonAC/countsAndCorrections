...
#~ 
		#~ for background in backgrounds:
			...
			#~ for subprocess in process.samples:
				...
				#~ for ev in treeMM:
					#~ 
					#~ print ev.pdgId1, ev.pdgId2
	
				#~ print treeEE.GetEntries(), treeMM.GetEntries(), treeEM.GetEntries()
		histEE, histMM, histOF = getHistograms(path,plot,runRange,isMC, backgrounds,region)	
		...
		if not "eta" in plot.variable and not "nJets" in plot.variable and not "nBJets" in plot.variable: 
			number = float(plot.yaxis.split("/")[1].split("GeV")[0])*4
			
			plot.yaxis = "Events / %s GeV"%number
			histEE.Rebin(4)
			histMM.Rebin(4)
			histOF.Rebin(4)
		...
		plotPad.RedrawAxis()	
		hCanvas.Print("fig/sameSign_%s_%s_%s_%s.pdf"%(selection.name,runRange.label,plot.variablePlotName,plot.additionalName))				
