...
if (__name__ == "__main__"):
	...
	if len(argv) > 2:
		if argv[2] == "MC":
			path = "/home/jan/Trees/sw538v0478/sw538v0478.processed.TTJets_MGDecays_madgraph_Summer12.root"
			#~ label = label+"_MC"
			suffix = suffix+"_MC"
	...
	treeOF = readTreeFromFile(path, "EMu")
	treeEE = readTreeFromFile(path, "EE")
	treeMM = readTreeFromFile(path, "MuMu")
#~ 
	...
	#~ cuts = "weight*(chargeProduct < 0 && %s && p4.M() > 20  && met > 100 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	...
	sfHist = eeHist.Clone()
	sfHist.Add(mmHist)
	print sfHist.Integral(sfHist.FindBin(20),sfHist.FindBin(70))+sfHist.Integral(sfHist.FindBin(120),sfHist.FindBin(1000))
	sfHist.Divide(emHist)
	print emHist.Integral(emHist.FindBin(20),emHist.FindBin(70))+emHist.Integral(emHist.FindBin(120),emHist.FindBin(1000))
