...
def getErrHist(plot,combination,region,ofHist,dyHist,rSFOFErr):
	...
	if combination == "SF":
		localZPred = zPredictions.legacy.SF
		localROutIn = rOutIn
	#~ elif combination == "EE":
...
def makeResultPlot(path,selection,runRange,cmsExtra,edgeShape=False,edgeShapeMC=False,differentEdgePositions=False):
	...
	if region == "inclusive":
		histSFDY.Scale(zPredictions.legacy.SF.central.val + (zPredictions.legacy.SF.forward.val) / histSFDYScale.Integral(histSFDYScale.FindBin(81),histSFDYScale.FindBin(101)))
		...
		histSFDY.Scale(getattr(zPredictions.legacy.SF,region).val / histSFDYScale.Integral(histSFDYScale.FindBin(81),histSFDYScale.FindBin(101)))
	...
	makePlot(histSF,histOFSF,selection,plot,runRange,region,cmsExtra,"SF",histSFDY,edgeShape=edgeShape,edgeShapeMC=edgeShapeMC,differentEdgePositions=differentEdgePositions)
