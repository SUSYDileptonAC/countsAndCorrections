...
if (__name__ == "__main__"):
	setTDRStyle()
	path = "/home/jan/Trees/sw532v0474/Blinded/"
	...
	cutsBlockA = "weight*(chargeProduct < 0 && %s  && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3 && runNr <= 201678 && !(runNr >= 198049 && runNr <= 198522) && !(runNr == 195649 && lumiSec == 49 && eventNr == 75858433) && !(runNr == 195749 && lumiSec == 108 && eventNr == 216906941) )"%(ptCut,etaCut)
	cutsBlockB = "weight*(chargeProduct < 0 && %s  && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3 && (runNr > 201678 || (runNr >= 198049 && runNr <= 198522)) )"%(ptCut,etaCut)
	print cutsBlockA
	print cutsBlockB
	nEvents=-1
...
#~ 
	...
	for name, tree in EMutrees.iteritems():
		...
		if name == SampleName:
			print SampleName
			EMuhistBlockA = createHistoFromTree(tree,  variable, cutsBlockA, nBins, firstBin, lastBin, nEvents)
			EMuhistBlockB = createHistoFromTree(tree,  variable, cutsBlockB, nBins, firstBin, lastBin, nEvents)
	...
	#~ ratioGraphs =  ratios.RatioGraph(EMuhistBlockA,EMuhistBlockB, xMin=20, xMax=300,title="Block A / Block B",yMin=0.0,yMax=2,ndivisions=10,color=ROOT.kGreen+3,adaptiveBinning=100000)

	#~ ratioGraphs.draw(ROOT.gPad,True,False,True,chi2Pos=0.8)
	
	hCanvas.Print("OFUnblinding.pdf")
	
	
	cutsBlockA = "weight*(chargeProduct < 0 && %s && p4.M() > 20 && p4.M() < 70 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3 && runNr <= 201678 && !(runNr >= 198049 && runNr <= 198522) && !(runNr == 195649 && lumiSec == 49 && eventNr == 75858433) && !(runNr == 195749 && lumiSec == 108 && eventNr == 216906941) && !(runNr == 190646 || runNr == 190659 || runNr == 190679 || runNr == 190688 || runNr == 190702 || runNr == 190703 || runNr == 190706 || runNr == 190707 || runNr == 190708 || runNr == 190733 || runNr == 190736 || (runNr == 191271 && lumiSec >= 159) || runNr == 193192 || runNr == 193193) )"%(ptCut,etaCut)
	cutsBlockB = "weight*(chargeProduct < 0 && %s && p4.M() > 20 && p4.M() < 70 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3 && (runNr > 201678 || (runNr >= 198049 && runNr <= 198522)) )"%(ptCut,etaCut)
	nBins = 50
	firstBin = 20
	lastBin = 70
	for name, tree in EMutrees.iteritems():
		if name == SampleName:
			...
			EMuhistBlockA = createHistoFromTree(tree,  variable, cutsBlockA, nBins, firstBin, lastBin, nEvents)
			EMuhistBlockB = createHistoFromTree(tree,  variable, cutsBlockB, nBins, firstBin, lastBin, nEvents)
			treeBlockA = tree.CopyTree(cutsBlockA)
	blockA = EMuhistBlockA.Integral()
	blockB = EMuhistBlockB.Integral()

	print blockA, blockB
	
	for ev in treeBlockA:
