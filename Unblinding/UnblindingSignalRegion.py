...
if (__name__ == "__main__"):
	...
	if argv[1] == "Barrel":
		log.logHighlighted("Welcome to Unblinding. Now Unblinding the Central Signal Region. Stay Tuned")
	
	elif argv[1] == "Endcap":
		log.logHighlighted("Welcome to Unblinding. Now Unblinding the Forward Signal Region. Stay Tuned")
	...
	cuts = "weight*(chargeProduct < 0 && %s && p4.M() > 20 && p4.M() < 70 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	cutsBTagged = "weight*(chargeProduct < 0 && %s && p4.M() > 20 && p4.M() < 70 && nBJets >= 1 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	cutsBTaggedVeto = "weight*(chargeProduct < 0 && %s && p4.M() > 20 && p4.M() < 70 && nBJets == 0 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	#~ cuts = "weight*(chargeProduct < 0 && %s && p4.M() > 120  && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	#~ cutsBTagged = "weight*(chargeProduct < 0 && %s && p4.M() > 120  && nBJets >= 1 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	#~ cutsBTaggedVeto = "weight*(chargeProduct < 0 && %s && p4.M() > 120  && nBJets == 0 && ((met > 100 && nJets >= 3) ||  (met > 150 && nJets >=2)) && %s && deltaR > 0.3)"%(ptCut,etaCut)
	nEvents=-1
	...
	for name, tree in EEtrees.iteritems():
		if name == SampleName:
			...
			eeTreeBTagged = tree.CopyTree(cutsBTagged)
			eeTreeBTagVeto = tree.CopyTree(cutsBTaggedVeto)
	for name, tree in MuMutrees.iteritems():
		if name == SampleName:
			...
			mumuTreeBTagged = tree.CopyTree(cutsBTagged)
			mumuTreeBTagVeto = tree.CopyTree(cutsBTaggedVeto)			
	for name, tree in EMutrees.iteritems():
		if name == SampleName:
			...
			emuTreeBTagged = tree.CopyTree(cutsBTagged)
			emuTreeBTagVeto = tree.CopyTree(cutsBTaggedVeto)			
	...
	hCanvas.Print("Unblinding_SignalRegion_%s.pdf"%suffix)
	

	ee = eeTree.GetEntries()
	eeBTagged = eeTreeBTagged.GetEntries()
	eeBTagVeto = eeTreeBTagVeto.GetEntries()
	mm = mumuTree.GetEntries()
	mmBTagged = mumuTreeBTagged.GetEntries()
	mmBTagVeto = mumuTreeBTagVeto.GetEntries()
	em = emuTree.GetEntries()
	emBTagged = emuTreeBTagged.GetEntries()
	emBTagVeto = emuTreeBTagVeto.GetEntries()
	#~ print sf, of
	
	log.logHighlighted("Block B has been Unblinded:")
	log.logHighlighted("ee: %d mm: %d sf: %d of: %d"%(ee,mm,ee+mm,em))
	log.logHighlighted("B-tagged ee: %d mm: %d sf: %d of: %d"%(eeBTagged,mmBTagged,eeBTagged+mmBTagged,emBTagged))
	log.logHighlighted("B-tag Veto ee: %d mm: %d sf: %d of: %d"%(eeBTagVeto,mmBTagVeto,eeBTagVeto+mmBTagVeto,emBTagVeto))
	
	
	#~ for ev in eeTreeBTagVeto:
		#~ print "%s:%s:%s"%(ev.runNr,ev.lumiSec,ev.eventNr)
	#~ print "----------------------------"
	#~ for ev in mumuTreeBTagVeto:
		#~ print "%s:%s:%s"%(ev.runNr,ev.lumiSec,ev.eventNr)
	#~ print "----------------------------"
	for ev in emuTreeBTagged:
		#~ print "%s:%s:%s"%(ev.runNr,ev.lumiSec,ev.eventNr)
		if ev.runNr == "201668" and ev.runNr == "150" and ev.eventNr=="170503986":
			print ev.bjet1pt
		if ev.runNr == "204113" and ev.runNr == "339" and ev.eventNr=="511676668":
			print ev.bjet1pt
