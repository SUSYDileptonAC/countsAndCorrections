...
if (__name__ == "__main__"):
	...
	for name, tree in EEtrees.iteritems():
		...
		if argv[3] == "MC":
			if name == sampleName2:
				EEhist2 = createHistoFromTree(tree,  variable, cuts, nBins, firstBin, lastBin, nEvents)
	for name, tree in MuMutrees.iteritems():
		if name == sampleName:
			MuMuhist = createHistoFromTree(tree,  variable, cuts, nBins, firstBin, lastBin, nEvents)
		if argv[3] == "MC":
			if name == sampleName2:
				MuMuhist2 = createHistoFromTree(tree,  variable, cuts, nBins, firstBin, lastBin, nEvents)			
	for name, tree in EMutrees.iteritems():
		if name == sampleName:
			EMuhist = createHistoFromTree(tree,  variable, cuts, nBins, firstBin, lastBin, nEvents)
		if argv[3] == "MC":
			if name == sampleName2:
				EMuhist2 = createHistoFromTree(tree,  variable, cuts, nBins, firstBin, lastBin, nEvents)			
	
	if argv[3] == "MC":
		EEhist.Add(EEhist2)
		EMuhist.Add(EMuhist2)
		MuMuhist.Add(MuMuhist2)
	...
	if argv[2] == "SF":	
		...
		if "TMath" in cuts:
			nllPredictionScale = 1.05
			snllPredictionScale = 0.05
		else:
			nllPredictionScale = 1.07
			snllPredictionScale = 0.06				
			
	elif argv[2] == "EE":
		SFhist = EEhist.Clone()	
		if "TMath" in cuts:
			nllPredictionScale = 0.46
			snllPredictionScale = 0.04
		else:
			nllPredictionScale = 0.50
			snllPredictionScale = 0.03		
	else:
		SFhist = MuMuhist.Clone()	
		if "TMath" in cuts:
			nllPredictionScale = 0.58
			snllPredictionScale = 0.05
		else:
			nllPredictionScale = 0.56
			snllPredictionScale = 0.03	
	...
	continuum = (SFhist.Integral(SFhist.FindBin(120+0.01),SFhist.FindBin(1000)) - EMuhist.Integral(EMuhist.FindBin(120+0.01),EMuhist.FindBin(1000))*nllPredictionScale )
	continuumError = sqrt(sqrt(SFhist.Integral(SFhist.FindBin(120+0.01),SFhist.FindBin(1000)))**2 + sqrt(EMuhist.Integral(EMuhist.FindBin(120+0.01),EMuhist.FindBin(1000))*nllPredictionScale)**2) 
	...
	for index in range (0,metRange):
		...
		line2 = ROOT.TLine(1000,0,1000,SFhist.GetBinContent(SFhist.GetMaximumBin()))
		...
		Labelout.DrawLatex(125,SFhist.GetBinContent(SFhist.GetMaximumBin()/10),"Out")
		...
		print SFhist.Integral(SFhist.FindBin(120+0.01),SFhist.FindBin(1000)), EMuhist.Integral(EMuhist.FindBin(120+0.01),EMuhist.FindBin(1000)) , nllPredictionScale 	
	...
	errorband.GetYaxis().SetRangeUser(0.0,0.2)
	...
	latex2.SetNDC(True)
	if argv[3] == "MC":
		latex2.DrawLatex(0.2, 0.96, "CMS Simulation  #sqrt{s} = 8 TeV,     #scale[0.6]{#int}Ldt = %sfb^{-1}"%lumi)
	else:
