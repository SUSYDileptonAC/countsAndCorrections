...
if (__name__ == "__main__"):
	...
	for name, tree in eeTrees.iteritems():
		if name == SampleName:
			eeHist = createHistoFromTree(tree, "p4.M()", cuts, 60, 0, 30, nEvents = -1)
			#~ eeHist.Scale(1./tree.GetEntries())
	for name, tree in mmTrees.iteritems():
		if name == SampleName:
			mmHist = createHistoFromTree(tree, "p4.M()", cuts, 60, 0, 30	, nEvents = -1)
			#~ mmHist.Scale(1./tree.GetEntries())
	for name, tree in emTrees.iteritems():
		if name == SampleName:
			emHist = createHistoFromTree(tree, "p4.M()", cuts, 60, 0, 30	, nEvents = -1)
	...
	latex.DrawLatex(0.95, 0.96, "19.8 fb^{-1} (8 TeV)")
	cmsExtra = "Private Work"

	latexCMS.DrawLatex(0.18,0.955,"CMS")
	latexCMSExtra.DrawLatex(0.32,0.955,"%s"%(cmsExtra))				
	...
	#~ ratioPad.cd()
#~ 
	#~ ratioGraphs =  ratios.RatioGraph(eeHist,emHist, 0, 100,title="e^{#pm}e^{#mp} / #mu^{#pm}#mu^{#mp}",yMin=0.0,yMax=2,ndivisions=10,color=ROOT.kRed,adaptiveBinning=0.25)
#~ 
	#~ ratioGraphs.draw(ROOT.gPad,True,False,True,chi2Pos=0.8)	
	#~ line3 = ROOT.TLine(20,0,20,2)
	#~ line3.SetLineColor(ROOT.kBlue+3)
#~ 
	#~ line3.SetLineWidth(2)
	#~ line3.SetLineStyle(2)
#~ 
	#~ line3.Draw("same")	
	hCanvas.Print("mllJustification_Data.pdf")
