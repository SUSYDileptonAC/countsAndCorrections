...
if (__name__ == "__main__"):
	...
	ptCutLabel = "20"#"20(10)"
	variable = "p4.M()"
	cuts = "(chargeProduct < 0 && %s  && abs(eta1) < 2.4 && abs(eta2) < 2.4 && deltaR > 0.3 && p4.M() > 20)"%(ptCut)
	...
	for name, tree in eeTrees.iteritems():
		if name == SampleName:
			eeHist = createHistoFromTree(tree, "abs(eta2)", cuts, 25, 0, 2.5, nEvents = -1)
			eeHist.Scale(1./eeHist.GetEntries())
	for name, tree in mmTrees.iteritems():
		if name == SampleName:
			mmHist = createHistoFromTree(tree, "abs(eta2)", cuts, 25, 0, 2.5, nEvents = -1)
	...
	yMin=0
	yMax = max(eeHist.GetBinContent(eeHist.GetMaximumBin()),mmHist.GetBinContent(mmHist.GetMaximumBin()))*1.5
	hCanvas.DrawFrame(0,yMin,2.5,yMax,"; %s ; %s" %("|#eta^{leading}|","N_{events} / 0.1 [a.u.]"))
	ROOT.gStyle.SetOptStat(0)
	...
	line1 = ROOT.TLine(1.4,0,1.4,yMax)
	...
	ratioGraphs =  ratios.RatioGraph(eeHist,mmHist, 0, 2.5,title="e^{#pm}e^{#mp} / #mu^{#pm}#mu^{#mp}",yMin=0.0,yMax=2,ndivisions=10,color=ROOT.kRed,adaptiveBinning=0.25)

	ratioGraphs.draw(ROOT.gPad,True,False,True,chi2Pos=0.8)	
	hCanvas.Print("gapJustification.pdf")
