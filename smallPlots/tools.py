...
def myLegend(x1, y1, x2, y2, borderSize=1):
    ...
    legend.SetFillColor(myColors['White'])
    return legend
...
def storeParameter(project, task, name, value):
    basePath = "/Volumes/Data/SUSY/Pickles/"
    fileName = "%s-%s-%s.pkl" % (project, task, name)
    pFile = open("%s%s" % (basePath, fileName), 'wb')
    pickle.dump(value, pFile)
    pFile.close()

def loadParameter(project, task, name):
    basePath = "/Volumes/Data/SUSY/Pickles/"
    fileName = "%s-%s-%s.pkl" % (project, task, name)
...
def storeData(project, key, value):
    basePath = "/Volumes/Data/SUSY/ShelveData/"
