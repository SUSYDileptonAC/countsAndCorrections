...
if (__name__ == "__main__"):
	...
	for name, tree in eeTrees.iteritems():
		if name == SampleName:
			eeHist = createHistoFromTree(tree, "p4.M()", cuts, 50, 0, 100, nEvents = -1)
			eeHist.Scale(1./eeHist.GetEntries())
	for name, tree in mmTrees.iteritems():
		if name == SampleName:
			mmHist = createHistoFromTree(tree, "p4.M()", cuts, 50, 0, 100	, nEvents = -1)
			mmHist.Scale(1./mmHist.GetEntries())
	for name, tree in emTrees.iteritems():
		if name == SampleName:
			emHist = createHistoFromTree(tree, "p4.M()", cuts, 50, 0, 100	, nEvents = -1)
	...
	hCanvas.DrawFrame(0,yMin,100,yMax,"; %s ; %s" %("m(ll)","N_{events} / 2 GeV [a.u.]"))
	ROOT.gStyle.SetOptStat(0)
	...
	ratioGraphs =  ratios.RatioGraph(eeHist,mmHist, 0, 100,title="e^{#pm}e^{#mp} / #mu^{#pm}#mu^{#mp}",yMin=0.0,yMax=2,ndivisions=10,color=ROOT.kRed,adaptiveBinning=0.25)
	...
	line3.Draw("same")	
	hCanvas.Print("mllJustification_eeVSmm.pdf")
