...
def getHistogram(path,dilepton,plot,runRange,background, isMC = True):
        if "NLL" in path:
                plot.cuts = plot.cuts.replace(" && metFilterSummary > 0", "")
        ...
        if isMC:
                ...
                if dilepton == "SF":
                        hist = proc.createCombinedHistogram(runRange.lumi,plot,treesEE,treesMM) 
                elif dilepton == "DF":
                        hist = proc.createCombinedHistogram(runRange.lumi,plot,treesEM) 
                        
                return hist                
        else:
                
                histDF = getDataHist(plot, treesEM)
                return histDF
...
def createVariableDistributions(selection, plotName, useNLL = False):
        ...
        for runRangeName in runRanges.allNames:
                ...
                histDF = getHistogram(path,"DF",plot,runRange,"Data", False)
                if first:
                        histDataDF = histDF.Clone()
                else:
                        histDataDF.Add(histDF, 1.0)
                        
                plot = getPlot(plotName)
                plot.addRegion(selection)
                plot.addRunRange(runRange)   
                histSF = getHistogram(path,"SF",plot,runRange,"TT_Powheg")
                if first:
                        histTTSF = histSF.Clone()
                else:
                        histTTSF.Add(histSF, 1.0)
                 
                plot = getPlot(plotName)
                plot.addRegion(selection)
                plot.addRunRange(runRange)   
                histDF = getHistogram(path,"DF",plot,runRange,"TT_Powheg")
                if first:
                        histTTDF = histDF.Clone()
                else:
                        histTTDF.Add(histDF, 1.0)
                 
                      
                plot = getPlot(plotName)
                plot.addRegion(selection)
                plot.addRunRange(runRange)  
                histSF = getHistogram(path,"SF",plot,runRange,"RareOnZ")
                if first:
        ...
        histDataDF.Scale(1. / histDataDF.Integral())
        histTTSF.Scale(1. / histTTSF.Integral())
        histTTDF.Scale(1. / histTTDF.Integral())
        ...
        plotDistribution(histDataDF, histTTSF, histTTDF, histZNu, sigPoints, sigHists, selection, plot)
...
def plotDistribution(histDataDF, histTTSF, histTTDF, histZNu, sigPoints, sigHists, selection, plot):
        
        template = plotTemplate()
        
        histTTSF.SetLineColor(ROOT.kBlack)
        histTTSF.SetFillStyle(0)
        histTTSF.SetMarkerSize(0)
        histTTSF.SetLineWidth(3)
        
        print histTTSF.Integral(1, histTTSF.FindBin(23.9))
        
        histTTDF.SetLineColor(ROOT.kBlue)
        histTTDF.SetFillStyle(0)
        histTTDF.SetMarkerSize(0)
        histTTDF.SetLineWidth(3)
        ...
        # maximum = histTTSF.GetMaximum()
        # if histDY.GetMaximum() > maximum:
                # maximum = histDY.GetMaximum()
        # if histZNu.GetMaximum() > maximum:
                # maximum = histZNu.GetMaximum()
        
        template.setPrimaryPlot(histDataDF, "PE", label="Data (DF)")
        template.addSecondaryPlot(histTTSF, "hist", label="t#bar{t} (SF)")
        template.addSecondaryPlot(histTTDF, "hist", label="t#bar{t} (SF)")
        template.addSecondaryPlot(histZNu, "hist", label="Z+#nu (SF)")
        # template.redrawPrimary = False
        
        leg = ROOT.TLegend(0.54,0.65,0.9,0.9)
        leg.SetBorderSize(0)
        leg.SetFillStyle(0)
        ...
        for point in sigPoints:
                ...
                template.addSecondaryPlot(sigHists["%s_%s_%s"%(point[0], point[1], point[2])][0], "hist")
                leg.AddEntry(sigHists["%s_%s_%s"%(point[0], point[1], point[2])][0], sigHists["%s_%s_%s"%(point[0], point[1], point[2])][1],"l")
        
        template.hasLegend = True
        template.plotData = True
        template.legendPosX1 = 0.37
        template.legendPosX2 = 0.52
        template.legendPosY1 = 0.7
        ...
        template.latexCuts.posY = 0.50
        template.latexCuts.align = 12
        
        template.logY = False
        template.minimumY = 0
        template.maximumScale = 1.4
        ...
        line1.DrawLine(24, 0, 24, histDataDF.GetMaximum()/3)
...
def main():
        ...
        createVariableDistributions(getRegion("SignalDeltaPhi"), "nLLPlot", True) 
        createVariableDistributions(getRegion("SignalInclusiveHighMT2"), "nLLPlot", True) 
