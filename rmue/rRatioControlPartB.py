...
class Ratio_nJets_MC(): #nJets
	...
	cut=Cuts.basicOhneNJetPlusInvMassPlusMet50CutPartB
...
class Ratio_nJets_Data():
	...
	processes8TeV=[Processes8TeV.DataReprocessed]
...
class Ratio_nVertices_MC(): #nVertices
	...
	cut=Cuts.basicPlusInvMassPlusMet50CutPartB
...
class Ratio_id1_MC(): #id1
	variable = "id1"
	filename = "r-Abh_id1"
	cut=Cuts.basicPlusInvMassPlusMet50CutPartB+" && id2 < 0.05"
...
class Ratio_id2_MC(): #id2
	variable = "id2"
	filename = "r-Abh_id2"
	cut=Cuts.basicPlusInvMassPlusMet50CutPartB+" && id1 < 0.05"
	binning=[-0.01+i*0.01 for i in range(0,17)]
	processes=[Processes7TeV.TTJets, Processes7TeV.ZJets,Processes7TeV.SingleT,Processes7TeV.DiBoson]
	processes8TeV=[Processes8TeV.TTJets]
	xaxis=Titles.id2
...
class Ratio_pt2_MC(): # p_t1
	variable = "pt2"
	filename = "r-Abhpt2"
	cut=Cuts.basicPlusInvMassCutPartB
	#binning=range(0,60,20)+range(60,100,5) # 7TeV
	binning=range(20,30,5)+range(30,60,5)+range(60,100,10)
	processes=[Processes7TeV.TTJets, Processes7TeV.ZJets,Processes7TeV.SingleT,Processes7TeV.DiBoson]
	processes8TeV=[Processes8TeV.TTJets]
...
class Ratio_DeltaR_MC(): #m_ll
	...
	cut=Cuts.basicCutNoDeltaRPartB
...
class Ratio_p4M_MCIsoSideband(): #m_ll
	...
	cut=Cuts.basicCutIsoSidebandPartB
	#binning=range(15,255,5)
	binning=range(20,60,10)+range(60,120,10)+range(120,250,25)
	processes=[Processes7TeV.TTJets, Processes7TeV.ZJets,Processes7TeV.SingleT,Processes7TeV.DiBoson]
	processes8TeV=[Processes8TeV.TTJets]
...
#	SubProcesses8TeV.DataReprocessedPath = "../Daten/"
	plotpath="fig/"
	rootContainer = []
...
#	SubProcesses8TeV.DataReprocessedPath = "../Daten/"
	...
	# variante 2 = ohne slope
		...
		filename="8TeVrRatioDataVsMCControl_PartB_%s" %variable
		linearMC = ROOT.TF1("linear", "[0]", xMin , xMax)
		linearMC.SetLineColor(Colors.mc)
		linearData = ROOT.TF1("linear", "[0]", xMin , xMax)
		linearData.SetLineColor(Colors.data)
		if plotMC==Ratio_id2_002_MC:
			filename="8TeVrRatioDataVsMCControl_PartB_2_id2_002"
		elif plotMC==Ratio_id1_002_MC:
			filename="8TeVrRatioDataVsMCControl_PartB_2_id1_002"
		elif plotMC==Ratio_p4M_MC:
			filename="8TeVrRatioDataVsMCControl_PartB_mll"
		elif plotMC==Ratio_met_MC_tt:
			filename = "8TeVrRatioDataVsMCControl_PartB_met-tt"
		elif plotMC==Ratio_p4M_MCtt:
			filename="8TeVrRatioDataVsMCControl_PartB_mll_tt"
		elif plotMC==Ratio_p4M_MCZ:
			filename="8TeVrRatioDataVsMCControl_PartB_mll_Z"
		elif plotMC==Ratio_DeltaR_MCZ:
			filename="8TeVrRatioDataVsMCControl_PartB_DeltaR_Z"
		elif plotMC==Ratio_p4M_MCSS:
			filename="8TeVrRatioDataVsMCControl_PartB_mll_SS"
		elif plotMC==Ratio_p4M_MCIsoSideband:
			filename="8TeVrRatioDataVsMCControl_PartB_mll_IsoSideBand"
		elif plotMC==Ratio_p4M_MCControlSS:
			filename="8TeVrRatioDataVsMCControl_PartB_mll_SSControl"
	else:
...
def tableRatio(n_mumu_R3MC, n_ee_R3MC, rR3MC, sigma_rR3MC, sigma_rR3MC_syst_p, sigma_rR3MC_syst_m, 
	...
	if len(selectionModifier) >0:
		for modifier in selectionModifier:			
			nameModifier = nameModifier+modifier
	savename="Tables/new_r-ValuesControl_PartB%s.txt"%nameModifier
...
def main():
	...
	if argv[1] == "CentralValue":
		n_mumu_R3Data, n_ee_R3Data, rR3Data, rR3Data_stat, rR3Data_up, rR3Data_down = calculateRatio(Cuts.basicPlusInvMassPlusMet50CutPartB, Processes8TeV.DataReprocessed,selectionModifier)
		n_mumu_R3MC, n_ee_R3MC, rR3MC, rR3MC_stat, rR3MC_up, rR3MC_down = calculateRatio(Cuts.basicPlusInvMassPlusMet50CutPartB, Processes8TeV.ZJets,selectionModifier)
	#
